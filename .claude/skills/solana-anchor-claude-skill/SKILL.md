...
- Before declaring success or celebrating, run `bun test`. If the tests fail, there is more work to do. Don't stop until `bun test` passes on the code you have made.
...
- Do not use yarn, npm, or pnpm. Use bun. Replace yarn/npm/pnpm with bun everywhere you see it.
...
- **Testing**: How to run the tests (e.g., `bun test`)
...
- Use the Node.js inbuilt test and assertion libraries (then start the tests using `bun` instead of `ts-mocha` or `tsx`)
